...
function undoColour(){
  if(!colorFills.length){ setStatus('No colour fills to undo'); return; }
  saveState();
  colorFills.pop();
  redrawCanvas();
  setStatus(`Colour fill removed (${colorFills.length} remaining)`);
}

// Clear only colour fills — keeps erase fills, crop mask, image position intact
function clearColours(){
  if(!colorFills.length){ setStatus('No colour fills to clear'); return; }
  saveState();
  colorFills=[];
  redrawCanvas();
  setStatus('All colour fills cleared');
}
...
function resetAnchor(){
  ...
  setStatus('Anchor reset — scale and rotate now pivot at canvas centre');
}
...
canvas.addEventListener('mousedown',e=>{
  ...
  if(currentTool==='anchor'){
    ...
    ['anchorBtn','pAnchorBtn'].forEach(id=>{const el=document.getElementById(id);if(el) el.classList.remove('active');});
    setStatus('Anchor pinned — next scale/rotate will pivot around this point');
    return;
  }
...
function fitToCrop(){
  ...
  function step(now){
    ...
    else setStatus('Fitted to crop region — use Scale slider to fine-tune');
  }
...
function wandApply(action){
  ...
  if(action==='fill'||action==='erase'){
    ...
    updateFitCropBtn();wandClose();redrawCanvas();setStatus('Wand crop mask applied');
  }
}
