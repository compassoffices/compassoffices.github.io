...
function pickCustomColor(){
  ...
  setStatus('Custom colour: '+v);
}

// Undo only the last colour fill (leaves erase fills, crop, etc. untouched)
function undoColour(){
  if(!colorFills.length){ setStatus('元に戻すカラー塗りつぶしはありません'); return; }
  saveState();
  colorFills.pop();
  redrawCanvas();
  setStatus(`カラー塗りつぶしを削除しました (残り ${colorFills.length})`);
}

// Clear only colour fills — keeps erase fills, crop mask, image position intact
function clearColours(){
  if(!colorFills.length){ setStatus('クリアするカラー塗りつぶしはありません'); return; }
  saveState();
  colorFills=[];
  redrawCanvas();
  setStatus('すべてのカラー塗りつぶしをクリアしました');
}
...
function resetAnchor(){
  ...
  setStatus('アンカーをリセット — スケール・回転の中心がキャンバス中央に戻りました');
}
...
function toggleSnap(){
  ...
  setStatus(snapEnabled?'エッジスナップ オン':'エッジスナップ オフ');
}
  ...
  pdfDoc.getPage(n).then(page=>{
    ...
    page.render({canvasContext:tc.getContext('2d'),viewport:vp}).promise.then(()=>{
      ...
      uploadedImage.onload=()=>{
        ...
        applyFileSizeIfSelected();
        redrawCanvas(); saveState(true); setStatus(`PDF ${n}ページ: ${uploadedImage.width}×${uploadedImage.height}`);
      };
...
function undo(){
  if(!undoStack.length){setStatus('元に戻す操作はありません');return;}
...
function redo(){
  if(!redoStack.length){setStatus('やり直す操作はありません');return;}
...
canvas.addEventListener('mousedown',e=>{
  ...
  if(currentTool==='anchor'){
    ...
    ['anchorBtn','pAnchorBtn'].forEach(id=>{const el=document.getElementById(id);if(el) el.classList.remove('active');});
    setStatus('アンカーを固定 — 次のスケール/回転はこの点を中心に行われます');
    return;
  }
  if(['rectangle','selection','color','rotateSel'].includes(currentTool)){
    ...
    setStatus(`${penMode==='erase'?'消去':'カラー'}ペン: ${penPoints.length}点${s.snapped?' [スナップ]':''}`);
  }
});
...
canvas.addEventListener('mouseup',e=>{
  ...
  if(currentTool==='rectangle') eraseFills.push({type:'rect',x:nx,y:ny,width:w,height:h});
    ...
    setStatus('オレンジのハンドルをドラッグして回転 · ✔ 完了したら適用');
    return;
  }
  currentRect=null;redrawCanvas();
  setStatus(currentTool==='rectangle'?`消しゴムボックス (${eraseFills.length})`:currentTool==='selection'?'クロップマスクを適用しました':`カラー塗りつぶし (${colorFills.length})`);
});
      ...
      if(currentRect){
        ...
        if(w>=6&&h>=6){
          ...
          if(currentTool==='rectangle') eraseFills.push({type:'rect',x:nx,y:ny,width:w,height:h});
            ...
            currentRect=null; redrawCanvas();
            showRotWidget({x:nx,y:ny,w,h});
            setStatus('ハンドルをドラッグして回転 · ✔ 完了したら適用');
            return;
          }
          setStatus(currentTool==='rectangle'?`消しゴムボックス`:currentTool==='selection'?'Crop applied':`カラー塗りつぶしを追加しました`);
        }
        currentRect=null; redrawCanvas();
      }
    }
...
function fitToCrop(){
  ...
  function step(now){
    ...
    else setStatus('クロップ領域に合わせました — スケールスライダーで微調整できます');
  }
...
function clearAll(){
  ...
  setStatus('キャンバスをクリア — 塗りつぶし削除、元の画像を復元');
}
function downloadImage(){
  if(!uploadedImage){alert('先にフロアプランをアップロードしてください');return;}
  const[pw,ph]=currentPixelSize();
  ...
  dc.toBlob(b=>{
    ...
    setStatus(`${pw}×${ph}px JPGをダウンロードしました`);
  },'image/jpeg',.97);
}
...
function applyRotate(){
  ...
  newImg.onload = () => {
    ...
    cancelRotate(true);
    redrawCanvas();
    setStatus(`回転を適用しました (${prevAngle.toFixed(1)}°)`);
  };
  newImg.src = dataUrl;
}
...
function wandApply(action){
  ...
  if(action==='fill'||action==='erase'){
    ...
    newImg.onload=()=>{
      ...
      edgeData=null;wandClose();redrawCanvas();setStatus(`ワンド ${action} を適用しました`);
    };
    newImg.src=dataUrl;
  } else if(action==='crop'){
    saveState(false);
    ...
    updateFitCropBtn();wandClose();redrawCanvas();setStatus('ワンドのクロップマスクを適用しました');
  }
}
