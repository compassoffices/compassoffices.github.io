  ...
  try {
    for(let i = 0; i < PDF_QUEUE.length; i++){
      ...
      if(page1El){
        ...
        let _extras=[];
        try{
          if(typeof captureExtraMasterHtml==='function' && EXTRA_MASTERS.length){
            _extras=captureExtraMasterHtml();
            await _waitForCardReady();
          }
...
function printSlide(){
  ...
  const extraPages = (typeof captureExtraMasterHtml==='function') ? captureExtraMasterHtml() : [];
...
function downloadCurrentJSON(){
  ...
  const data = {
    ...
    extra_masters: (typeof EXTRA_MASTERS!=='undefined'?EXTRA_MASTERS:[]).map(m=>({url:(typeof _stripCb==='function'?_stripCb(m.url):m.url),label:m.label||''})),
