...
function covBuildWorld(scene){
  ...
  // ---- lighting (warm interior + soft shadows) ----
  scene.add(new THREE.HemisphereLight(0xfff3e0, 0x5a4b3b, 0.95));
  const dir = new THREE.DirectionalLight(0xffeacb, 0.75);
  dir.position.set(28, 45, 18);
  dir.castShadow = true;
  dir.shadow.mapSize.set(2048, 2048);
  const scam = dir.shadow.camera;
  scam.left = L.origin[0] - 2; scam.right = L.origin[0] + L.gw*L.cell + 2;
  scam.top = -(L.origin[1] - 2); scam.bottom = -(L.origin[1] + L.gh*L.cell + 2);
  scam.near = 1; scam.far = 120;
  dir.shadow.bias = -0.002;
  scene.add(dir);
  scene.add(dir.target);
  dir.target.position.set(L.origin[0] + L.gw*L.cell/2, 0, L.origin[1] + L.gh*L.cell/2);
  ...
  const texWalnut = canvasTex(256, (ctx, S) => {
    ctx.fillStyle = "#4B3728"; ctx.fillRect(0, 0, S, S);
    for (let i = 0; i < 46; i++){
      const x = (i*17 + (i*i)%13) % S;
      const w = 2 + (i*7) % 6;
      ctx.fillStyle = (i % 3 === 0) ? "rgba(30,20,12,.4)" :
                      (i % 3 === 1) ? "rgba(120,84,52,.35)" : "rgba(90,62,38,.4)";
      ctx.fillRect(x, 0, w, S);
    }
    for (let i = 0; i < 200; i++){
      ctx.fillStyle = "rgba(0,0,0,.1)";
      ctx.fillRect(Math.random()*S, Math.random()*S, 1, 6 + Math.random()*20);
    }
  });
  ...
  // walls: crisp white where the scan reads light; walnut wood grain where it reads dark
  if (L.wallC){
    const lightR = [], darkR = [], darkC = [];
    for (let i = 0; i < L.walls.length; i++){
      const c = new THREE.Color("#" + (L.wallC[i] || "F4F1EA"));
      const lum = 0.299*c.r + 0.587*c.g + 0.114*c.b;
      if (lum > 0.42) lightR.push(L.walls[i]);
      else { darkR.push(L.walls[i]); darkC.push(L.wallC[i]); }
    }
    _covInstanced(scene, lightR, M_WALL, 0, L.wallH, false);
    const M_WOODWALL = new THREE.MeshLambertMaterial({ map: texWalnut, color: 0xffffff });
    _covInstanced(scene, darkR, M_WOODWALL, 0, L.wallH, false, darkC, 0.45);
  } else {
  ...
  const _leafG = (() => {
    const g = new THREE.PlaneGeometry(0.16, 0.55).toNonIndexed();
    g.translate(0, 0.27, 0);   // pivot at leaf base
    return g;
  })();
  function addItem(f, builder){
    const local = [];
    const lp = (geo,color,px,py,pz,sx,sy,sz,ry,rx) =>
      local.push({geo,color,px,py,pz,sx,sy,sz,ry:ry||0,rx:rx||0});
    ...
    for (const p of local){
      const m = new THREE.Matrix4().compose(
        ...
        new THREE.Quaternion().setFromEuler(new THREE.Euler(p.rx, p.ry, 0)),
  ...
  const B = {
    ...
    plant(lp, LW, LD, f){
      // tropical planter with fanned leaf fronds (like the real window planters)
      const s = Math.max(.3, Math.min(LW, LD));
      lp(_cylG, POT, 0, .17, 0, s*.6, .34, s*.6);
      lp(_cylG, shade(POT,.7), 0, .345, 0, s*.52, .02, s*.52);   // soil
      const nleaf = 12;
      for (let i = 0; i < nleaf; i++){
        const ang = i/nleaf * Math.PI*2 + (i%3)*.4;
        const tilt = -(.55 + (i%4)*.14);                          // lean outward
        const scale = .8 + ((i*13)%5)*.14;
        lp(_leafG, (i%2) ? LEAF1 : LEAF2,
           Math.cos(ang)*s*.12, .32, Math.sin(ang)*s*.12,
           s*scale, s*scale*1.4, 1, ang + Math.PI/2, tilt);
      }
    },
    cabinet(lp, LW, LD, f){
  ...
  // shadows: solid geometry casts and receives (sky/snow/glass excluded)
  scene.traverse(o => {
    if ((o.isMesh || o.isInstancedMesh) && o.material && !o.isPoints){
      if (o.material.transparent || o.material.fog === false) return;
      o.castShadow = true; o.receiveShadow = true;
    }
  });
}

/* ---- festive third-person avatar ---- */
